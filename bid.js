...
$(document).on('pageinit', '#bid', function() {
	...
	$('#bid #selected-driver').find('a').text(F2013.race.selectedDriver.name);
...
var BidValidator = function() {
	...
	$('select').change(function(event) {
		if (validator.isValid()) {
			$('#indsend').removeClass('ui-disabled');
		}	else {
			$('#indsend').addClass('ui-disabled');
		}
	});
	...
	function changeTheme(select, newTheme) {
		var previousTheme = select.parent().attr('data-theme') || 'a';
		if (newTheme !== previousTheme) {
			select.parent()
				.attr('data-theme', newTheme)
				.removeClass('ui-btn-'+previousTheme)
				.addClass('ui-btn-'+newTheme);
		}		
	}
};
